...
実行方法(リポジトリ直下 = ~/wk_tool で):
    ...
    #   grep -r SuperSecret99 logs/ | grep -v env_overlay.sh   ← 何も出ない
    # (env_overlay.sh のみ実行に必要なため平文。実行前確認にも注意が表示される)

    # ステップ2から再開(直近実行の env_overlay.sh から TOKEN を復元):
...
直近実行の env_overlay.sh から TOKEN が復元されるため、このステップ単体でも動く。
